...
{
	Sprite sprite(Texture(),IntRect(0, 0, shape.Width, shape.Height));
...
bool SimpleUI::Panel(int32_t id, BOARD shape, std::vector<bool>& state)
{
	...
	Sprite box(Texture(), IntRect(0, 0, (shape.RectWidth + SPACE) * shape.SizeX + SPACE, (shape.RectHeight + SPACE) * shape.SizeY + SPACE));
	...
	for (int32_t y = 0; y < shape.SizeY; y++)
	{
		for (int32_t x = 0; x < shape.SizeX; x++)
		{
			...
			if (TestPointInRect(RECT{posX,posY,shape.RectWidth,shape.RectHeight}) && SimpleWindow::GetInstance().GetMouseButtonState(Mouse::Left))
			{
				state[y*shape.SizeX + x] = true;
			}
...
void SimpleUI::Text(int32_t id, float X, float Y, std::string text, int32_t fontSize, Color color, Font * font)
{
	sf::Text output;
	output.setFont(*font);
...
bool SimpleUI::TestPointInRect(RECT shape) const
{
